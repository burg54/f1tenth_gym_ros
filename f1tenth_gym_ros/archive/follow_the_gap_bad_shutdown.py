...
class StopNode(Node):
    def __init__(self):
        ...
        self.timer = self.create_timer(0.1, self.publish_stop_command) # Publish periodically
    ...
    def publish_stop_command(self):
        ...
        self.get_logger().info('Publishing stop command: "Speed=0.0"')
'''
...
def main(args=None):
    ...
    try:
        ...
        pass
        '''
        follow_the_gap.stop_drive()
        follow_the_gap.get_logger().info('Ctrl+C received, stopping car.')
        stop_msg = AckermannDriveStamped()
        stop_msg.drive.speed = 0.0 
        stop_msg.drive.steering_angle = 0.0
        '''
        #follow_the_gap.pub_drive(0.0,0.0)
        #rclpy.spin_once(follow_the_gap.pub_drive(0.,0.), timeout_sec=0.1)
        #GymBridge.drive_callback(stop_msg)
        #robot.stop_motors() # Function to set motor commands to zero
    ...
    '''
    try:
        rclpy.spin(follow_the_gap) # This will block until Ctrl+C or shutdown() is called
    except KeyboardInterrupt:
        pass # Handle Ctrl+C gracefully
    finally:
        #follow_the_gap.pub_drive(0.,0.)
        print("hi cody!!!!!!")
        follow_the_gap.destroy_node()
        #rclpy.shutdown()
        '''
